...
void loop()
{
  ...
  {
    if (currentMillis - temp_hum_previousMillis > temp_hum_interval)
    { // process once per interval
      ...
      if (average_counter < 6)
      {
        ...
        Serial.print(F("Hum: "));
        Serial.println(average_hum[average_counter]);
        Serial.print(F("Temp: "));
        Serial.println(average_temp[average_counter]);
        average_counter++;
      ...
      else
      {
        ...
        Serial.print(F("Average Hum: "));
        Serial.println((average_hum[0] + average_hum[1] + average_hum[2] + average_hum[3] + average_hum[4] + average_hum[5]) / 6);
        Serial.print(F("Average Temp: "));
        Serial.println((average_temp[0] + average_temp[1] + average_temp[2] + average_temp[3] + average_temp[4] + average_temp[5]) / 6);
  ...
  //****************************************************************************
  if (currentMillis - previousMillis > water_heater_interval)
  { // READ ONLY ONCE PER water_heater_interval
    ...
    if (water_heater_state == HIGH)
    { //if the NO contact on the leak break detector closes, a leak is present, send data to server
